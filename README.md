...
## Update `Signup` Route on `App` Component

Update the `Route` for `/signup` like so:

```jsx
<Route path='/signup' 
    render={ (props) => <Signup {...props} nowCurrentUser={nowCurrentUser} /> } />
```
